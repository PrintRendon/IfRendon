    ...
    def notify(self, args):
        ...
        try:
            ...
            if selectionInput.selectionCount > 0:
                ...
                ang :adsk.fusion.ModelParameter = joint.angle
                An_1 = ang.value 
                
                for v in range(int(math.degrees(An_1)),abs(int(Angle_1)),2):
                    if abs(int(math.degrees(An_1))) > abs(int(Angle_1)):
            ...
            if selectionInput.selectionCount > 0:
                ...
                joint2 :adsk.fusion.Joint = selection6
                ang2 :adsk.fusion.ModelParameter = joint2.angle
                An_2 = ang2.value
                
                for v2 in range(int(math.degrees(An_2)),abs(int(Angle_2)),2):
                    if abs(int(math.degrees(An_2))) > abs(int(Angle_2)):
                        joint2.angle.expression = str(-v2)
                        app.activeViewport.refresh()
                    else:
                        joint2.angle.expression = str(v2)
                        app.activeViewport.refresh()
...
def getAngle1(point1, point2, point3, point4, Xposition, Yposition) -> float:
    try:
        ...
        Alpha = math.degrees(math.atan2(Yposition,Xposition))
...
def getAngle2(point1, point2, point3, point4, Xposition, Yposition) -> float:
    try:
        ...
        Alpha = math.atan2(Yposition,Xposition)

        Beta = math.acos((( L1 * L1) - (L2 * L2) + (Hip * Hip))/(2 * L1 * Hip))

        #First Angle to insert in first joint
        Angl1 = math.degrees(Alpha + Beta)

        Gamma = math.acos(((L1 * L1) + (L2 * L2) - (Hip * Hip))/ (2 * L1 * L2))

        #Second Angle to insert in second joint
        AngL2 = (math.degrees(Gamma)- 180)
